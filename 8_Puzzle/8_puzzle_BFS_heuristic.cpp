...
int misplacedTiles(const State &curr, const State &goal) {
    int count = 0;
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            if(curr.s[i][j] != 0 && curr.s[i][j] != goal.s[i][j])
                count++;
        }
    }
    return count;
...
vector<State> genMove(const State &current, int &id_counter, const State &goal) {
    int row, col;
    ...
    for(int d=0; d<4; d++){
        ...
        if(new_r >= 0 && new_r < 3 && new_c >= 0 && new_c < 3){
            ...
            new_state.h = misplacedTiles(new_state, goal);
...
void solve(State start, State goal){
    ...
    start.id = id_counter;
    start.parent_id = 0;
    start.h = misplacedTiles(start, goal);
    ...
    while(!pq.empty()){
        ...
        vector<State> moves = genMove(curr, id_counter, goal);
